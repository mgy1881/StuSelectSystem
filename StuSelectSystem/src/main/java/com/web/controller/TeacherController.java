...
//    }
    ...
    @GetMapping("/page")
    public Result getTeacherListPage(TeacherQuery teacherQuery) {
        PageVO<TeacherVO> teacherVOPageVO = teacherService.selectAllOrByMsgPage(teacherQuery);
        return Result.success(teacherVOPageVO);
    }

    //管理员根据条件获取教师信息
    @GetMapping
    public Result getTeacherInfo(TeacherQuery teacherQuery) {
        List<Teacher> teacherList = teacherService.selectAllOrByMsg(teacherQuery);
        return Result.success(teacherList);
    }
